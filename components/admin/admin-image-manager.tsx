...
export function AdminImageManager({
  productId,
  images,
}: Readonly<{ productId: string; images: ImageItem[] }>) {
  ...
  return (
    <section className="surface-card p-5 sm:p-7">
      ...
      <p className="mt-1 text-sm text-[#5b665f]">
        Upload a JPG, PNG, or WebP image. The first image is used as the
        storefront primary image.
      </p>
      <form
        action={action}
      ...
      >
        ...
        <div className="flex items-end">
          <button
            ...
            disabled={pending}
          >
            <ImagePlus size={16} /> {pending ? "Uploading…" : "Upload image"}
          </button>
        </div>
        {state.message ? (
      ...
      {images.length ? (
        <ul className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
          {images.map((image, index) => (
            <li
              ...
              key={image.id}
            >
              <div className="relative aspect-square bg-[#e7ebe0]">
                <Image
        ...
        <p className="mt-6 rounded-2xl bg-[#f7f8f2] p-5 text-sm text-[#5b665f]">
          No images yet. Upload the first product photo above.
        </p>
      )}
    </section>
  );
