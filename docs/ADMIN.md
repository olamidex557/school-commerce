...
Current catalogue routes are `/admin/products`, `/admin/products/new`, `/admin/products/[id]`, and `/admin/categories`. They support product/category creation and editing, active/archive state, featured state, category counts, active variants, integer-minor-unit prices, stock quantities, image upload/removal, and a public storefront preview link. Variants omitted from a saved edit are archived (`is_active=false`) rather than deleted. Categories cannot be archived while they have active products. Product deletion is intentionally absent.
