...
`CI` runs for pull requests to `main` and direct pushes to `main`. It uses Node 22, `npm ci`, linting, strict TypeScript checks, all Vitest tests, a production Next build, immutable-migration safety checks, repository secret scanning, server-secret boundary checks, and a production-dependency audit at high severity. Build-only placeholder public Supabase values are supplied by the workflow; no real service-role or Paystack credential is available to CI.
