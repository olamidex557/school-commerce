...
Phase 7 adds `202608190001_admin_order_operations.sql` and Phase 8 adds `202608190002_delivery_pickup_operations.sql`. Apply them after Phase 6 before using `/admin/orders`; then run `supabase/tests/rls-policy-verification.sql`. Phase 7 removes direct authenticated administrator writes to orders, customers, items, and payment attempts and replaces them with an audited operational status RPC. Phase 8 adds only the delivery-dispatch enum value and method-aware transition guard. Do not use a table editor or client code to alter payment truth, immutable order snapshots, or stock.
