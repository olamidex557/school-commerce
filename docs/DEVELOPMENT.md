...
Phase 7 adds `202608190001_admin_order_operations.sql`. Apply it after the Phase 6 migrations before using `/admin/orders`; then run `supabase/tests/rls-policy-verification.sql`. It is an intentional security tightening: it removes direct authenticated administrator writes to orders, customers, items, and payment attempts and replaces them with an audited operational status RPC. Do not use a table editor or client code to alter payment truth, immutable order snapshots, or stock.
